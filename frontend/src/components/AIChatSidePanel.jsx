...
export default function AIChatSidePanel() {
  ...
  const checkOllamaStatus = async ({ initial = false } = {}) => {
    ...
    try {
      const response = await cryptoAPI.ollamaStatus({ timeout: 10000 })
      setOllamaStatus({
  ...
  return (
    <>
      ...
      {/* Side Panel */}
      <div className={`ai-chat-side-panel ${isOpen ? 'open' : ''}`}>
        ...
        <div className="chat-footer">
          <small>💡 Ask about portfolio, buying power, Bitcoin, Ethereum, trends, or analysis</small>
        </div>
      </div>
    </>
  )
